               ...
               <form method="post" action="{{ url('dashboard/data-siswa', $siswa->id) }}">
                  ...
                  <div class="form-group">
                        ...
                        <select name="kelas" class="custom-select @error('kelas') is-invalid @enderror" {{ count($kelas) == 0 ? 'disabled' : '' }}>
                           ...
                           @else
                                 @foreach($kelas as $value)
                                    <option value="{{ $value->id }}" {{ $siswa->id_kelas == $value->id ? 'selected' : '' }}>{{ $value->nama_kelas.' - '. $value->angkatan }}</option>
                                 @endforeach
                           @endif
                       </select>
                     </div>
                     ...
                     <div class="input-group mb-3">
                        ...
                        <select class="custom-select @error('jk') is-invalid @enderror" name="jk">
                  ...
                  <div class="form-group">
                     ...
                     <input type="number" class="form-control @error('nomor_telepon') is-invalid @enderror" name="nomor_telepon" value="{{ $siswa->nomor_telp }}">
